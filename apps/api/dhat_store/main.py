import hashlib,hmac,json,secrets,os
...
app=FastAPI(title='DHĀT STORE API',version='2.0.0')
...
class ProviderIn(BaseModel): name:str; priority:int=100; timeout_seconds:int=15; enabled:bool=True; config_key:str|None=None
class MappingIn(BaseModel): service_id:int; provider_id:int; external_service_id:str; priority:int=100; enabled:bool=True

@app.get('/health')
async def health(): return {'status':'ok','service':'dhat-store'}
@app.get('/api/v1/health/ready')
async def ready(db:AsyncSession=Depends(get_db)):
    await db.execute(select(func.count()).select_from(User)); return {'status':'ready'}
...
async def telegram_auth(p:TelegramAuth,db:AsyncSession=Depends(get_db)):
    ...
    token=secrets.token_urlsafe(48); db.add(Session(user_id=u.id,token_hash=hashlib.sha256(token.encode()).hexdigest(),expires_at=datetime.now(timezone.utc)+timedelta(days=settings.session_days))); await db.commit()
    return {'access_token':token,'token_type':'bearer','user':{'id':u.id,'telegram_id':u.telegram_id,'username':u.username,'role':u.role}}
...
async def create_category(p:CategoryIn,db:AsyncSession=Depends(get_db),u:User=Depends(require_roles('OWNER','ADMIN'))):
    x=Category(**p.model_dump()); db.add(x); await db.flush(); await audit(db,u,'category.create',x.id,p.model_dump()); await db.commit(); return {'id':x.id}
...
async def wallet_transactions(limit:int=Query(50,ge=1,le=100),db:AsyncSession=Depends(get_db),u:User=Depends(current_user)):
    q=select(Ledger).join(Wallet).where(Wallet.user_id==u.id).order_by(Ledger.id.desc()).limit(limit); return [{'id':x.id,'amount':str(x.amount),'type':x.type,'reference':x.reference,'created_at':x.created_at} for x in (await db.execute(q)).scalars()]
...
async def quote(p:OrderIn,db:AsyncSession=Depends(get_db),u:User=Depends(current_user)):
    ...
    rules=(await db.execute(select(PricingRule).where(PricingRule.enabled.is_(True),or_(PricingRule.service_id==s.id,PricingRule.category_id==s.category_id),PricingRule.currency.is_(None)| (PricingRule.currency==s.currency)))).scalars().all(); unit=calculate_price(s.price,rules); total=unit*p.quantity; discount=Decimal('0')
    if p.coupon_code:
...
async def list_tickets(db:AsyncSession=Depends(get_db),u:User=Depends(current_user)):
    q=select(SupportTicket).where(SupportTicket.user_id==u.id) if u.role=='CUSTOMER' else select(SupportTicket)
    return [{'id':t.id,'subject':t.subject,'priority':t.priority,'status':t.status,'created_at':t.created_at,'updated_at':t.updated_at} for t in (await db.execute(q.order_by(SupportTicket.id.desc()))).scalars()]
